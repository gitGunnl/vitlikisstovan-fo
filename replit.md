...
This is a static website for Vitlíkisstovan, an AI and coding education platform with a Faroese-first approach. The application is built as a modern single-page application with React, deployed as a static site on Replit. It features a landing page that showcases educational programs, consulting services, and resources. The platform is designed to provide professional AI and coding education through a comprehensive 12-week flagship program and various consulting services. Contact form submissions are handled directly through Google Forms (no-cors POST). A lightweight Express backend exists for the /val election API and for form-submission monitoring (synthetic health checks every 15 min + client-side failure beacons), which sends alert emails via the Replit Gmail integration when a lead form silently breaks. Monitoring state is persisted to JSON files under `data/` for direct inspection. Required env vars for monitoring: `ALERT_EMAIL_TO` and an authorized Gmail connection.
