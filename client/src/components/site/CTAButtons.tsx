...
export default function CTAButtons({ primary, secondary, className }: CTAButtonsProps) {
  ...
  return (
    <div className={`flex flex-col gap-4 justify-center animate-fade-in ${className || ''}`}>
      {primary && (
        <div className="flex flex-col items-center gap-3">
          {isPrimaryLong && (
            <p className="text-white/95 text-sm sm:text-base text-center leading-relaxed max-w-md px-4">
              {primary.text}
            </p>
          )}
      ...
      {secondary && (
        <Button asChild variant="outline" className="group bg-white/15 backdrop-blur-sm border-2 border-white/40 text-white hover:bg-white/25 hover:border-white/60 h-auto py-3 px-6 sm:py-4 sm:px-8 text-sm sm:text-base whitespace-normal text-center leading-tight min-h-[48px] flex items-center justify-center rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 font-medium">
