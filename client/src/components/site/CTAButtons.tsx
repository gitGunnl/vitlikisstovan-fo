...
export default function CTAButtons({ primary, secondary, className }: CTAButtonsProps) {
  return (
    <div className={`flex flex-col sm:flex-row gap-4 justify-center ${className || ''}`}>
      <Button asChild size="lg" className="shiny-btn" data-testid={primary.testId}>
        <a href={primary.href}>
          {primary.text}
        </a>
      </Button>
      {secondary && (
        <Button variant="outline" asChild size="lg" className="shiny-btn" data-testid={secondary.testId}>
